...
服务降级:当服务不可用(相当于当服务器正在等待的时候,网络延迟,服务器响应慢),客户端一直在等待,应该直接返回一个错误提示给客户端.使用fallback方法返回当前服务不可用提示.
    作用:目的是为了提高用户体验(示例错误提示:服务器忙,请稍后重试) 防止服务雪崩效应.
    比如:连接超时 网络延迟 服务器响应

服务熔断:目的是为了保护服务,在高并发的情况下,如果达到一定极限(可以自己设置阈值),如果流量超出了设置阈值,然后直接拒绝访问,保护当前服务.使用服务降级.服务熔断和服务降级一起使用.
服务熔断:防止访问请求过多流量过大(属于高并发),设置一个限制,比如最多只能同时访问100个请求,超出的请求存放在缓存队列中,如果缓存队列达到饱和,直接拒绝访问服务,直接无法访问服务.
服务熔断效果:防止服务雪崩 被挂掉.
...
服务限流:解决服务服务雪崩效应,使用服务隔离机制(线程池方式和信号量方式)
    ...
    信号量方式:滑动窗口计数器算法原理 滑动窗口计数器可以解决传统计数器临界问题


服务熔断与降级 
    信号量:使用一个原子计数器(或信号量)来记录当前有多少个线程在运行,当请求进来时先判断计数器的数值,若超过设置的最大线程个数则拒绝改请求,若不超过则通过放行.这时候计数器+1,请求返回成功后计数器-1.
...
令牌桶算法(Token)
    令牌桶为两个动作
        动作1:(固定速率往桶中存入令牌)
        动作2:(客户端如果想要访问请求,先从桶中获取token)

令牌桶算法优点:能接受突然高并发请求.

SpringBoot整合RateLimiter实现令牌桶算法
1.为什么要实现服务限流:
    秒杀抢购 双11 服务安全(流量攻击 DDOS) 雪崩效应
    防止流量的突然性
2.限流最终的目的是为了保护服务
3.限流算法
    a.计数器方式(传统计数器缺点:临界值问题 可能违背定义固定速率原则) --滑动窗口计数器方式
    b.令牌桶方式(token) guava限流令牌算法
    c.漏桶算法方式 使用网关
    d.应用层(Nginx) http_limit

滑动窗口计数器算法原理:创建6个独立的(格子),每个格子都有自己的独立计数器
eg:限制每60秒(分钟)内只能有10个请求,超过情况下直接拒绝访问服务.
    滑动窗口计数器可以解决传统计数器临界问题 每个格子独立计数10秒
    滑动次数:如果超过了一分钟之后,每隔10秒时间移动到下一个格子.整个滑动窗口只能包含6个格子.最终能够实现临界值在同一个60秒内.

使用注解方式使用RateLimiter
漏桶算法与令牌桶算法区别
Nginx(应用层)互联网安全架构里面

网关:拦截所有的接口实现服务限流 秒杀抢购 或者大流量的接口才会实现限流

1.传统方式整合RateLimiter有很大缺点:代码重复量特别大,而且本身也不支持注解方式
2.限流代码可以网关,相当于针对所有服务接口实现限流(排除法),维护性不是很强.
3.正常互联网公司项目,不是所有的服务接口都需要实现限流,一般限流主要针对大流量.
    比如秒杀抢购系统


AOP的作用:定义一个切入点,拦截切入点方法,拦截到方法后,可以在这个方法之前或者之后进行处理
    作用:简化重复代码

应用场景:事务底层实现 日志 权限控制 mybatis绑定SQL语言 性能检测 Spring容器注册注入


漏桶限流算法的原理:
    以固定的速率从桶里流出水滴,以任意速率往桶中放入水滴,桶容量大小是不会发生改变的.
    流入:以任意速率往桶中放入水滴
    流出:以固定速率从桶中流出水滴
    水滴:一个唯一不重复标识.
因为桶中的容量是固定的,如果流入水滴的速率大于流出水滴速率,那么桶中的水滴可能会发生溢出,那么溢出的水滴请求都是拒绝访问,或者直接调用服务降级方法.

漏桶算法与令牌桶算法区别:
    令牌桶算法:以固定的速率生成对应的令牌,客户端只需要在桶中获取到令牌后,就可以访问服务请求.
    漏桶算法:以任意速率往桶中放入水滴,如果桶中的水滴没有满的话,就可以访问服务.
在突发情况请求情况的时候,令牌中只需要客户端你能够拿到令牌就能够访问服务,但是漏桶算法和令牌桶算法相反


互联网安全架构
什么是防盗链技术:
    相当于限制资源(图片,视频,文件)只能在某个域名(限制服务器)来源进行访问

防盗链底层技术:使用http协议请求头中的Referer记录请求来源
使用过滤器拦截请求,判断请求头中的域名(Referer记录)于需要限制的域名访问是否一致的,如果不一致的情况下,说明可能被盗用.
...
防止伪造token请求,在互联网电商项目中,会话信息使用令牌方式保存.
如何防止使用抓包分析获取到令牌,用令牌伪造请求支付,下单等核心业务?
    互联网上没有绝对防止不能抓包分析到令牌,可以使用调用接口时候,确认是本人在操作.(发送短信验证码 or 图像识别方式)

SQL注入原理
    如果程序代码中,dao层的SQL语句使用拼接方式会造成SQL注入问题,拼接SQL语句成立执行
    在写程序中,dao层SQL不能写拼接方式,最好使用预编译方式使用#号

#和$的区别
    mybatis,SQL语句传递参数最好使用#,因为#会预编译SQL语句,可以有效防止SQL攻击,$需要SQL语句拼接,可能会受到SQL语句的攻击.
